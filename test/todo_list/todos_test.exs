defmodule TodoList.TodosTest do
  ...
  describe "items" do
    ...
    test "create_item/1 with valid data creates a item" do
      valid_attrs = %{content: "some content", completed_at: ~U[2025-07-09 04:33:00Z]}

      assert {:ok, %Item{} = item} = Todos.create_item(valid_attrs)
      assert item.content == "some content"
      assert item.completed_at == ~U[2025-07-09 04:33:00Z]
    end
    ...
    test "update_item/2 with valid data updates the item" do
      item = item_fixture()
      update_attrs = %{content: "some updated content", completed_at: ~U[2025-07-10 04:33:00Z]}

      assert {:ok, %Item{} = item} = Todos.update_item(item, update_attrs)
      assert item.content == "some updated content"
      assert item.completed_at == ~U[2025-07-10 04:33:00Z]
    end
